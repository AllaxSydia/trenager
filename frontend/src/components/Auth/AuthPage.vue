...
export default {
  ...
  data() {
    return {
      ...
      registerData: { username: '', email: '', password: '', confirmPassword: '', agreeToTerms: false },
      toast: { show: false, type: 'error', title: '', message: '', timeout: null }
    }
  },
  methods: {
    async apiRequest(url, body) {
      const res = await fetch(url, {  // Убрал http://localhost:3000
        method: 'POST',
    ...
    async handleLogin() {
      if (!this.loginData.email || !this.loginData.password) {
        this.showToast('error', 'Ошибка', 'Введите email и пароль');
        return;
      }
      this.isLoading = true;
      const { ok, data } = await this.apiRequest('/api/auth/login', this.loginData);
      if (!ok) {
    ...
    async handleRegister() {
      ...
      this.isLoading = true;
      const { ok, data } = await this.apiRequest('/api/auth/register', { username, email, password });
      if (!ok) {
